...
// Migrations are inlined to avoid bundling/asset-copy issues
const MIGRATIONS: Array<{ version: number; sql: string }> = [
  ...
  },
  {
    version: 2,
    sql: `
      ALTER TABLE synthesis ADD COLUMN notes TEXT;
      ALTER TABLE labels ADD COLUMN confluence_page_name TEXT;
    `,
  },
]
...
export function getDb(): Database.Database {
  if (!db) {
    const dbPath = join(app.getPath('userData'), 'recall.db')
