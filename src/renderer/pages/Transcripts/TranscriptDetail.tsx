...
export default function TranscriptDetail(): React.ReactElement {
  ...
  // Track which synthesis ID we've already loaded notes for to avoid re-initializing
  ...
  // On mount (or id change), seed synthesisPending from the main process so we correctly
  // show the spinner when the user navigates to this page mid-synthesis.
  useEffect(() => {
    if (!id) return
  ...
  // Combined effect: reset state on transcript navigation AND initialize notes once synthesis loads.
  // Using a single effect avoids the React effect-ordering race where the init effect fires
  // before the reset effect, setting notesInitialized=true, then reset clears notesValue and the
  // flag — but since synthesis dep didn't change, init never re-fires, leaving notes blank.
  useEffect(() => {
    // Detect navigation to a new transcript
    ...
    // Initialize notes once synthesis is available (and only once per synthesis ID)
          ...
          <Box sx={{ mt: 2, display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
            {synthesis.pushedToConfluence ? (
              ...
              <Button
                ...
                startIcon={pushMutation.isPending ? <CircularProgress size={16} /> : <OpenInNew />}
              >
                Push to Confluence
              </Button>
            )}
